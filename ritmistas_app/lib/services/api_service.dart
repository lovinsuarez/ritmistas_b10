...
class ApiService {
  ...
  Future<void> updateProfile(String token,
      {String? nickname, String? profilePic, DateTime? birthDate}) async {
  ...
  Future<String> redeemCode(String code, String token) async {
    final response = await http.post(Uri.parse('$_baseUrl/user/redeem'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
  ...
  Future<Map<String, dynamic>> getGeralRanking(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/ranking/geral'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200) return jsonDecode(response.body);
    throw Exception('Erro ranking geral');
  }

  Future<Map<String, dynamic>> getSpecificSectorRanking(
      String token, int sectorId) async {
    final response = await http.get(
        Uri.parse('$_baseUrl/ranking/sector/$sectorId'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200) return jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> getRankingForSector(
          String token, int sectorId) async =>
      getSpecificSectorRanking(token, sectorId);

  // --- LIDER ---
  Future<void> createActivity(String token,
      {required String title,
      String? description,
      required String type,
      String? address,
      required DateTime activityDate,
      required int pointsValue}) async {
    ...
    if (response.statusCode != 200 && response.statusCode != 201)
      throw Exception('Falha ao criar atividade');
  }

  Future<List<Activity>> getActivities(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/lider/activities'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
  ...
  Future<List<UserAdminView>> getSectorUsers(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/lider/users'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
  ...
  Future<List<UserAdminView>> getPendingUsers(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/lider/pending-users'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
          .map((json) => UserAdminView.fromJson(json))
  ...
  Future<void> approveUser(String token, int userId) async {
    final response = await http.put(
        Uri.parse('$_baseUrl/lider/approve-user/$userId'),
        headers: {"Authorization": "Bearer $token"});
  ...
  Future<void> rejectUser(String token, int userId) async {
    final response = await http.delete(
        Uri.parse('$_baseUrl/lider/reject-user/$userId'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode != 200 && response.statusCode != 204)
      throw Exception('Erro rejeitar');
  }

  Future<void> deleteUser(String token, int userId) async {
    final response = await http.delete(
        Uri.parse('$_baseUrl/lider/users/$userId'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode != 200 && response.statusCode != 204)
      throw Exception('Erro deletar');
  }
  ...
  Future<void> distributePoints(
      String token, int userId, int points, String description) async {
    final response = await http.post(
        Uri.parse('$_baseUrl/lider/distribute-points'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
        body: jsonEncode(
            {"user_id": userId, "points": points, "description": description}));
    if (response.statusCode != 200) throw Exception('Erro distribuir');
  }

  Future<void> createGeneralCode(String token,
      {required String codeString, required int pointsValue}) async {
    final response = await http.post(Uri.parse('$_baseUrl/lider/codes/general'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
        body: jsonEncode(
            {"code_string": codeString, "points_value": pointsValue}));
  ...
  Future<void> addBudget(String token, int liderId, int points) async {
    final response = await http.post(Uri.parse('$_baseUrl/admin-master/budget'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
  ...
  Future<void> createBadge(
      String token, String name, String description, String iconUrl) async {
    final response = await http.post(Uri.parse('$_baseUrl/admin-master/badges'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
  ...
  Future<List<Badge>> getAllBadges(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/admin-master/badges'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
  ...
  Future<void> awardBadge(String token, int userId, int badgeId) async {
    final response = await http.post(
        Uri.parse('$_baseUrl/admin-master/award-badge'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
  ...
  Future<List<Sector>> getAllSectors(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/admin-master/sectors'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
  ...
  Future<List<UserAdminView>> getAllLiders(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/admin-master/liders'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
          .map((json) => UserAdminView.fromJson(json))
  ...
  Future<List<UserAdminView>> getAllUsers(String token) async {
    final response = await http.get(Uri.parse('$_baseUrl/admin-master/users'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
          .map((json) => UserAdminView.fromJson(json))
  ...
  Future<void> promoteUserToLider(String token, int userId) async {
    final response = await http.put(
        Uri.parse('$_baseUrl/admin-master/users/$userId/promote-to-lider'),
        headers: {"Authorization": "Bearer $token"});
  ...
  Future<void> demoteLiderToUser(String token, int liderId) async {
    final response = await http.put(
        Uri.parse('$_baseUrl/admin-master/liders/$liderId/demote-to-user'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode != 200) throw Exception('Erro rebaixar');
  }

  Future<void> assignLiderToSector(
      String token, int sectorId, int liderId) async {
    final response = await http.put(
        Uri.parse('$_baseUrl/admin-master/sectors/$sectorId/assign-lider'),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token"
        },
  ...
  Future<List<dynamic>> getAuditLogs(String token) async {
    final response = await http.get(
        Uri.parse('$_baseUrl/admin-master/audit/json'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200) return jsonDecode(response.body);
    throw Exception('Erro audit');
  }

  Future<List<UserAdminView>> getUsersForSector(
      String token, int sectorId) async {
    final response = await http.get(
        Uri.parse('$_baseUrl/admin-master/sectors/$sectorId/users'),
        headers: {"Authorization": "Bearer $token"});
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
          .map((json) => UserAdminView.fromJson(json))
  ...
  Future<void> createAdminGeneralCode(String token,
      {String? codeString,
    ...
    if (codeString != null && codeString.isNotEmpty)
      body["code_string"] = codeString;
  ...
  Future<List<CodeDetail>> getAdminGeneralCodes(String token) async {
    ...
    if (response.statusCode == 200)
      return (jsonDecode(response.body) as List)
          .map((json) => CodeDetail.fromJson(json))
          .toList();
